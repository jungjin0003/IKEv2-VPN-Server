...
- 자동 차단 기능은 DSM 자동 차단 기준을 사용하며, 차단된 IP의 IKEv2 트래픽을
  패키지 방화벽에서 거부합니다.
...
하지만 SMB 서비스를 제공하려면 Samba에도 같은 NT 해시가 필요하기 때문에, DSM은 SMB 패스워드 저장소인 `/etc/samba/private/smbpasswd`에 계정별 NT 해시를 보관하고 있습니다. 때문에 해당 패키지는 이 저장소에서 NT 해시를 읽어와 DSM 사용자 계정 인증을 수행합니다.

- 서비스가 시작/재시작될 때 마다 SMB 패스워드 저장소에서 해시를 새로 읽어 계정을 등록하므로, 계정 추가, 삭제, 비밀번호 변경은 서비스를 재시작하였을 때 반영됩니다.
